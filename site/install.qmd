...
similar). That matters because the `quartobot resolve` pre-render hook
in `_quarto.yml` runs `quartobot` as a subprocess of `quarto render` —
so `quartobot` needs to be discoverable without venv activation, which
this install gives you.
...
can't find `quartobot` from a pre-render hook, the `~/.local/bin`
directory probably isn't on the PATH that Quarto inherits. Most
shells fix this with a login reload; in pinch you can hard-code
the full path in your `_quarto.yml`:

```yaml
project:
  pre-render: ~/.local/bin/quartobot resolve --from-scan . --output references.json --id-mode citation-key
```
